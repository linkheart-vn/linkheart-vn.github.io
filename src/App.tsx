...
type ProView = 'home' | 'dashboard' | 'management' | 'appointments' | 'wallet' | 'tracking' | 'footer-page' | 'reviews' | 'dating';
      ...
      >
        ...
        <div className="flex flex-col md:flex-row items-center justify-center gap-4">
          <button 
            ...
            className="w-full sm:w-auto px-8 py-4 md:px-10 md:py-5 bg-gray-50 text-gray-900 border-2 border-gray-200 font-black text-lg md:text-xl rounded-full hover:bg-white hover:border-blue-400 hover:text-blue-600 transition-all active:scale-95 flex items-center justify-center gap-3"
          >
...
const LinkyAI = ({ user }: { user: FirebaseUser | null }) => {
  ...
  useEffect(() => {
    ...
    const loadVoices = () => {
      if (synthesisRef.current) {
        ...
        if (availableVoices.length > 0) {
          ...
          if (voiceRetryInterval) clearInterval(voiceRetryInterval);
        }
      }
    };
    ...
    // Polling fallback to ensure voices load eventually on all browsers
    voiceRetryInterval = setInterval(loadVoices, 1000);
  ...
  // Buffer to prevent Garbage Collection in Chrome
  const currentUtterances = useRef<SpeechSynthesisUtterance[]>([]);

  const speak = (text: string) => {
    if (!synthesisRef.current || !isVoiceEnabled) {
      console.warn("Speech skipped: synthesisRef missing or voice disabled");
      return;
    }

    console.log("Linky is speaking:", text.substring(0, 30) + "...");
    ...
    const processNextChunk = () => {
      ...
      try {
        ...
        const viVoice = voicesRef.current.find(v => v.lang === 'vi-VN' || v.lang === 'vi_VN') || 
                        voicesRef.current.find(v => v.lang.includes('vi') || v.name.toLowerCase().includes('vietnamese'));
        ...
        utterance.onerror = (event: any) => {
          if (event.error !== 'interrupted' && event.error !== 'canceled') {
            console.error("Linky Speech Error:", event.error, event);
          }
          
          if (event.error === 'not-allowed') {
            console.error("Audio Blocked: Linky needs a user click to activate voice.");
          }
        ...
        console.error("Critical Speech Loop Exception:", err);
        setIsSpeaking(false);
      }
    };

    // Small timeout to allow engine cleanup
  ...
  useEffect(() => {
    ...
    const unlockAudio = () => {
      if (synthesisRef.current) {
        ...
        u.volume = 0.01; // Microscopic volume to prime the engine
  ...
  const handleSend = async (manualInput?: string) => {
    ...
    try {
      const systemInstruction = `Bạn là Linky, trợ lý AI thông minh của LinkHeart. 
        Mục tiêu: Tư vấn giải pháp chăm sóc gia đình, trẻ em và người già. 
        Ngôn ngữ: Tiếng Việt, ấm áp, ngắn gọn nhưng đầy đủ ý.
        Định dạng: Sử dụng Markdown (ngôi sao cho in đậm) để nhấn mạnh.`;
    ...
    <div className="theme-kids min-h-screen flex flex-col">
      ...
      <nav className="p-4 md:p-6 flex justify-between items-center bg-white border-b-4 border-kids-orange sticky top-0 z-50">
        <div className="flex items-center gap-2 cursor-pointer" onClick={() => setActiveView('home')}>
          <Baby className="text-kids-orange w-8 h-8 shrink-0" />
          <span className="text-xl md:text-2xl font-black text-kids-orange">LinkHeart<span className="hidden sm:inline"> Kids</span></span>
        </div>
            ...
            >
              <div className="grid lg:grid-cols-2 gap-8 md:gap-12 items-center mb-10 md:mb-20">
                <motion.div initial={{ x: -50, opacity: 0 }} animate={{ x: 0, opacity: 1 }}>
                  ...
                  <h1 className="text-4xl md:text-6xl font-black text-kids-orange mb-4 md:mb-6 leading-tight">Cùng bé <br /> vui chơi & học tập!</h1>
...
const ProMode = ({ 
  ...
  useEffect(() => {
    ...
    }, (err) => handleFirestoreError(err, OperationType.GET, `users/${auth.currentUser?.uid}/status/elderly`));
    return () => unsubscribe();
  }, []);
  ...
  return (
    <div className="theme-pro min-h-screen bg-pro-white flex flex-col">
      ...
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 py-4 sm:py-8 flex justify-between items-center sticky top-0 bg-pro-white/80 backdrop-blur-md z-50 w-full">
        <div className="flex items-center gap-2 cursor-pointer shrink-0 mr-2 md:mr-8" onClick={() => setActiveView('home')}>
          ...
          <span className="text-lg md:text-xl font-bold text-pro-green hidden sm:inline">LinkHeart Pro</span>
        </div>
        ...
        <div className="space-y-6">
          {modal.data?.chat ? (
            ...
            <div className="text-center space-y-4">
              ...
              <p className="text-gray-600 font-bold">{modal.content || modal.title}</p>
              <button 
                onClick={() => setModal({ ...modal, open: false })}
...
const ElderlyMode = ({ onBack }: { onBack: () => void }) => {
  ...
  return (
    <div className={`min-h-screen flex flex-col font-sans transition-all duration-500 pb-24 ${highContrast ? 'bg-black text-white' : 'bg-[#F8FAFC]'}`}>
        ...
        <div className="text-center p-6 space-y-8">
          {modal.data?.gallery ? (
            ...
            <>
              ...
              <h3 className="text-3xl font-extrabold text-gray-900 leading-tight">{modal.content || modal.title}</h3>
              <button 
                onClick={() => setModal({ ...modal, open: false })}
                className="w-full bg-primary text-white py-6 rounded-3xl font-bold text-2xl shadow-lg active:scale-95 transition-transform"
              >
      ...
      <nav className={`p-4 md:p-8 flex justify-between items-center border-b border-gray-200 sticky top-0 z-50 backdrop-blur-md ${highContrast ? 'bg-black border-white/20' : 'bg-white/80'}`}>
        <div className="flex items-center gap-4 cursor-pointer shrink-0" onClick={() => { setActiveView('home'); setFooterPage(null); }}>
          <div className="w-12 h-12 md:w-14 md:h-14 bg-primary rounded-2xl flex items-center justify-center shrink-0">
            <Heart className="text-white w-6 h-6 md:w-8 md:h-8" fill="currentColor" />
          </div>
          <div className="hidden sm:block">
            <span className="text-xl md:text-2xl font-black block leading-none text-gray-900">LinkHeart</span>
            <span className="text-[10px] md:text-xs font-bold text-primary uppercase tracking-[0.2em]">CHẾ ĐỘ CAO TUỔI</span>
          </div>
        </div>
            ...
            <motion.div key="home" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-10">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="md:col-span-2 bg-primary text-white p-8 rounded-[40px] shadow-lg flex flex-col justify-between relative overflow-hidden">
                  ...
                  <div>
                    <h2 className="text-4xl font-extrabold mb-2">Chào Bác, buổi sáng tốt lành!</h2>
                    <p className="text-xl font-medium opacity-90 mb-8 italic">"Hôm nay trời nắng đẹp, bác nhớ đi dạo nhé!"</p>
                  </div>
                  <div className="flex items-center gap-6">
                    <button 
                      onClick={handleVoiceAssistant}
                      className={`px-8 py-5 rounded-3xl font-bold text-xl flex items-center gap-4 shadow-2xl transition-all ${isListening ? 'bg-red-500 scale-95' : 'bg-white text-primary hover:scale-105'}`}
                    >
                ...
                <div className="bg-white p-8 rounded-[40px] shadow-lg border border-gray-100 flex flex-col items-center justify-center text-center">
                   <div className="relative mb-4">
                      <svg className="w-32 h-32 transform -rotate-90">
                        <circle cx="64" cy="64" r="58" stroke="currentColor" strokeWidth="12" fill="transparent" className="text-gray-100"/>
                        <circle cx="64" cy="64" r="58" stroke="currentColor" strokeWidth="12" fill="transparent" strokeDasharray="364.4" strokeDashoffset={364.4 - (364.4 * healthScore) / 100} className="text-primary"/>
                      </svg>
              ...
              <div className="mb-20">
                <div className="bg-blue-900/5 p-8 rounded-[40px] border-4 border-dashed border-blue-900/10 mb-8 flex items-center gap-6">
                   <div className="w-20 h-20 bg-blue-900 text-white rounded-3xl flex items-center justify-center shadow-xl shrink-0">
                      <Stethoscope className="w-10 h-10" />
                   </div>
                   <div>
                      <h4 className="text-xl font-black text-blue-900 uppercase">Lịch Kiểm tra sức khỏe quan trọng</h4>
                      <p className="text-gray-600 font-bold">Nhân viên LinkHeart sẽ thực hiện kiểm tra các chỉ số sinh tồn của bác vào trước <span className="text-blue-600 font-black">07:00 AM</span> và <span className="text-blue-600 font-black">06:00 PM</span> hàng ngày.</p>
                   </div>
                </div>
                <StaffSection 
...
// --- Pricing Component ---
const Pricing = ({ onBack, onSelectPlan, trialDaysLeft, activePlanId }: { onBack: () => void; onSelectPlan: (plan: Plan) => void; trialDaysLeft: number | null, activePlanId?: string }) => {
  const plans: Plan[] = [
    {
      id: "kids_1h",
      name: "LinkHeart Kids",
  ...
  return (
    <div className="min-h-screen bg-cream p-6 py-24 flex flex-col items-center relative overflow-hidden">
      ...
      >
        <h2 className="text-3xl md:text-5xl font-black mb-4">Các Gói Dịch Vụ</h2>
        <p className="text-lg md:text-xl text-gray-500 font-bold">Lựa chọn gói phù hợp để kết nối yêu thương bền vững.</p>
      </motion.div>
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-gray-900/40 backdrop-blur-xl">
      <motion.div 
        ...
        className="w-full max-w-md bg-white rounded-[48px] shadow-2xl p-10 text-center border-4 border-white"
      >
        <div className={`w-20 h-20 rounded-3xl mx-auto mb-8 flex items-center justify-center shadow-lg ${timeLeft > 0 ? 'bg-red-50 text-red-500' : 'bg-pro-green/10 text-pro-green'}`}>
        ...
        <h2 className="text-3xl font-black text-gray-900 mb-2 uppercase tracking-tight">
          {isSetup ? 'Thiết lập mã PIN' : 'Xác thực chủ sở hữu'}
        </h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="flex justify-center gap-3">
              <input 
                ...
                className="w-full text-center text-4xl font-black tracking-[0.5em] py-6 px-4 bg-gray-50 border-4 border-gray-100 rounded-3xl focus:border-pro-green outline-none transition-all placeholder:text-gray-200 placeholder:tracking-normal"
                autoFocus
              />
            </div>
            ...
            <button 
              ...
              className="w-full bg-gray-900 text-white py-6 rounded-3xl font-black text-xl uppercase tracking-widest shadow-xl hover:bg-black transition-all active:scale-95 disabled:opacity-30"
            >
